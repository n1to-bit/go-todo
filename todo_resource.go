...
func (tr *TodoResource) GetAllTodos(c *gin.Context) {
	...
	for rows.Next() {
		rows.Scan(&id, &created, &title, &description)
...
func (tr *TodoResource) GetTodo(c *gin.Context) {
	...
	err = tr.db.QueryRow("SELECT created, status, title, description FROM Todo WHERE id = ?", id).Scan(&created, &status, &title, &description)
